...
const NewsSection = () => {
  ...
  return (
    <div className='w-full bg-gray-100 py-12 mt-8 relative z-10 text-gray-900'>
      <div className='max-w-7xl mx-auto px-4'>
        {/* Section Header */}
        <div className='flex items-center justify-between mb-6'>
          <h2 className='text-3xl font-bold text-gray-900'> Latest News</h2>
          <Link to='/news' className='text-red-600 hover:text-red-700 font-medium flex items-center gap-1'>
            View All News <span className='text-xl'>→</span>
          </Link>
        </div>
        ...
        {!loading && (
          <>
            ...
            {breakingNews.length > 0 && (
              <div className='bg-red-600 rounded-xl p-4 mb-6 shadow-lg shadow-red-900/30'>
                <div className='flex items-center gap-3 overflow-hidden'>
                  <span className='bg-white text-red-600 text-sm font-bold px-3 py-1 rounded-full animate-pulse whitespace-nowrap'>
                    🔴 BREAKING
                  </span>
                  <div className='flex-1 overflow-hidden'>
                    <div className='animate-marquee whitespace-nowrap'>
                      {breakingNews.map((newsItem, index) => (
                        <span key={newsItem._id} className='text-white text-base font-medium mx-4'>
                          {newsItem.title}
            ...
            {/* News Grid - 4 columns with BIGGER TITLES */}
            {currentItems.length > 0 ? (
              <>
                ...
                {/* Navigation Arrows - Centered below grid */}
                {totalPages > 1 && (
